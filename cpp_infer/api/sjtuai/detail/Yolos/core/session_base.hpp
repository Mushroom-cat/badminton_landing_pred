...
inline std::vector<unsigned char> file_read_bin(const std::string& fileName)
{
...
private:
    void initSession(const std::string& modelPath, bool useGPU, int numThreads) {
        ...
        auto data = file_read_bin(modelPath);
        std::cout << modelPath << std::endl;
        std::cout << "data :" << data.size() << std::endl;
        //_session = new Ort::Session(env, data.data(), data.size(), _sessionOpt);
        // Load model
        session_ = Ort::Session(env_, data.data(), data.size(), sessionOptions_);
